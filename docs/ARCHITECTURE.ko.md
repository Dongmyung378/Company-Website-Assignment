...
| 영역 | 책임 |
| --- | --- |
| 루트 PHP 페이지 | 요청 처리, 접근 검사, 도메인 쿼리, 화면 렌더링 |
| `config/` | 환경 변수 기반 연결 설정과 공용 MySQLi 연결 |
| `assets/` | 페이지 스타일과 정적 이미지 |
...
현재 프로젝트는 비밀번호 해싱, 세션 인증, 주요 화면의 출력 이스케이프, 핵심 사용자 입력 쿼리의 값 바인딩을 적용했습니다. 실제 운영 전에 CSRF 방어, 일관된 입력 검증, 안전한 세션 쿠키 설정, 요청 속도 제한, 비밀 정보 관리, 데이터베이스 설치 경로 비공개 처리가 추가로 필요합니다.
